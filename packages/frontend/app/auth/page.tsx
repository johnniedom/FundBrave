...
const Page = () => {
  ...
  return (
    <div className="flex h-dvh bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      {/* MAINTAINER'S NOTE (Left Hero Section):
          This is our animated hero panel that showcases different content
          based on the current auth view. The overflow-hidden is crucial
          for the swipe animation to work properly. */}
      <motion.aside 
        className="hidden h-full w-1/2 py-10 lg:flex flex-col items-center justify-between overflow-hidden relative"
        initial={{ opacity: 0, x: -50 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.6, ease: "easeOut" }}
      >
        {/* MAINTAINER'S NOTE (Core Swipe Animation Container):
            This is the heart of our swipe animation. AnimatePresence handles
            the enter/exit lifecycle of components. When `view` changes,
            it detects the old component being removed and new one being added,
            then applies our defined animations accordingly. */}
        <div className="flex-1 flex items-center justify-center relative w-full">
          <AnimatePresence initial={false} custom={direction}>
            <motion.div
              key={view} // Critical: This tells React/Framer Motion this is a new component
              custom={direction}
